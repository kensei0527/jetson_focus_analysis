...
class ConcentrationMultiBranchMLP(nn.Module):
    def __init__(self, input_dim=390, hidden_dim=64, num_classes=4, dropout_p=0.5):
...
def load_mlp_model(model_path, device):
    model = ConcentrationMultiBranchMLP(input_dim=390, hidden_dim=64, num_classes=4, dropout_p=0.3)
...
def inference_loop():
    ...
    global latest_label, is_running, start_time
    ...
    parser.add_argument('--mlp_path', type=str, default='best_model.pth')
...
def api_reset_session():
    global session_data, latest_label
    print("[/api/reset_session] clearing session_data...")
    session_data.clear()
    latest_label = None  # これもリセットしたいなら
