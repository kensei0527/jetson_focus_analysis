...
"""
import sys
sys.path.append("/app/Pytorch_Retinaface")
...
# ======= グローバル変数 =======
latest_label = None   # 直近の 15秒分類 (0～3)
session_data = []     # 時系列データの格納: 各秒の blink, gaze_vel, label など
is_running = True     # スレッド停止制御用フラグ
start_time = time.time()  # セッション開始時刻
...
class ConcentrationMultiBranchMLP(nn.Module):
    def __init__(self, blink_dim=15, gaze_dim=15, au_dim=24, hidden_dim=64, num_classes=4, dropout_p=0.3):
        super(ConcentrationMultiBranchMLP, self).__init__()

        # blink branch
        self.blink_branch = nn.Sequential(
            nn.Linear(blink_dim, hidden_dim),
            nn.ReLU(),
        ...
        # au branches (24個ぶん ModuleList)
        self.au_branches = nn.ModuleList([
            nn.Sequential(
                nn.Linear(15, hidden_dim),
                nn.ReLU(),
                nn.Dropout(p=dropout_p)
            ) for _ in range(au_dim)
        ])

        # blink/gaze/au(24) => total (2+24)ブランチ
        # 各ブランチが hidden_dim を出力するので合計 (hidden_dim * (2 + 24))
        total_hidden_dim = hidden_dim * (2 + au_dim)

        # 全結合層を段階的に縮める
        self.fc1 = nn.Linear(total_hidden_dim, hidden_dim * 4)
    ...
    def forward(self, x):
        ...
        # au: x[:, 30:]
        # 24個のブランチに分割 => 1ブランチあたり15次元
        au = x[:, 30:]  # shape=(batch_size, 360)

        # branch forward
        blink_out = self.blink_branch(blink)  # (batch_size, hidden_dim)
        gaze_out = self.gaze_branch(gaze)     # (batch_size, hidden_dim)

        au_outs = []
        for i, branch in enumerate(self.au_branches):
            # au_feature = 15次元ずつ切り出し
            au_feature = au[:, i*15 : (i+1)*15]
            au_outs.append(branch(au_feature))
        # au_outs: list of (batch_size, hidden_dim)

        # 結合
        x_cat = torch.cat([blink_out, gaze_out] + au_outs, dim=1)
        # shape=(batch_size, hidden_dim*(2+au_dim))
...
def load_mlp_model(model_path, device):
    model = ConcentrationMultiBranchMLP(blink_dim=15, gaze_dim=15, au_dim=24, hidden_dim=24, num_classes=4, dropout_p=0.45)
    model.load_state_dict(torch.load(model_path, map_location=device))
    model.to(device)
    model.eval()
    return model

def classify_concentration(model, features_15sec, device):
    """
    features_15sec: shape (15, 26) のリストか np.array
       1秒ごとの [ blink, gaze_vel, (AU_neg/pos 24個) ] 合計26次元を15秒分
    """
    arr_15 = np.array(features_15sec, dtype=np.float32)  # (15, 26)
    arr_flat = arr_15.flatten()  # (15*26=390,)
    x = torch.tensor(arr_flat).unsqueeze(0).to(device)   # (1, 390)

    with torch.no_grad():
        out = model(x)  # (1,4)
        pred = torch.argmax(out, dim=1).item()  # 0..3
    return pred
...
def inference_loop():
    ...
    global latest_label, session_data, is_running, start_time
    ...
    parser.add_argument('--mlp_path', type=str, default='best_model2.pth')
    ...
    # --- MLPモデルをロード ---
    mlp_model = load_mlp_model(args.mlp_path, device)
    print("MLP model loaded from", args.mlp_path)
    ...
    # 15秒分の特徴量を貯めるバッファ
    feature_buffer = []  # 形: [[blink, gaze_vel, AU_24次元], ..., (15行)]
    au_list = []
        ...
        if frame_idx % step_size_for_au == 0:
            ...
            if faces and len(faces) > 0:
                ...
                if all_landmarks and len(all_landmarks)>0:
                    ...
                    # 2) flatten
                    au_scores_flat = au_scores.reshape(-1)      # shape: (24,)

                    # 3) Pythonのリストに
                    au_list = au_scores_flat.tolist()             # 長さ24のリスト
            ...
            # 1秒分の特徴量(合計26次元: blink_count, avg_vel, AU24個)
            row_features = [blink_count, avg_vel] + au_list  # 全26次元

            # バッファに追加
            feature_buffer.append(row_features)

            print(f"[Sec {frame_idx//step_size_for_au}] blink={blink_count}, gaze_vel={avg_vel:.3f}")

            # (B3) 15行(=15秒) 貯まったら MLPで推論
            if len(feature_buffer) == 15:
                pred_label = classify_concentration(mlp_model, feature_buffer, device)
                print("=== 15秒の集中度クラス:", pred_label, "===")

                # セッションデータに記録（グラフ表示用）
                # 今回は「15秒ごと」に1行として登録
                latest_label = pred_label
                now_sec = time.time() - start_time
                # 平均blink, gaze_velを計算(バッファ全体の平均とする簡易例)
                blink_array = [f[0] for f in feature_buffer]
                gaze_array  = [f[1] for f in feature_buffer]
...
def index():
    """
    メインHTML(templates/index.html) を返す
    """
...
def api_live_data():
    ...
    # デバッグ: session_data の先頭/末尾の数件を表示
    if len(session_data) > 0:
        print(f"[/api/live_data] session_data[0] = {session_data[0]}")
        print(f"[/api/live_data] session_data[-1] = {session_data[-1]}")
    else:
        print("[/api/live_data] session_data is empty.")
    """
    session_dataをJSONで返す。
    例:
    {
      "data": [
...
@app.route("/api/current_label")
def api_current_label():
    """
    直近15秒の分類結果(0~3)を返す
    { "label": 2 }
    """
